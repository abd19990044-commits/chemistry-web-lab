...
from .errors import ChecksumMismatchError, NotFoundError, OrchestratorError, ValidationError
...
def _verify_digest(document: dict, filename: str) -> bool:
    ...
    payload = {k: v for k, v in document.items() if k != "_digest"}
    actual1 = sha256_bytes(stable_json(payload).encode("utf-8"))
    if actual1 == recorded:
        return True
    # Compatibility with in-kernel runner default ensure_ascii
    actual2 = sha256_bytes(json.dumps(payload, sort_keys=True, separators=(",", ":"), default=str).encode("utf-8"))
    if actual2 == recorded:
        return True
    log_failure(
        log,
        ...
        filename=filename, expected=recorded[:16], actual=actual1[:16],
    )
    return False
...
def rebuild_from_kaggle(client: KaggleClient, job_id: str,
    ...
    for window in reversed(windows):
        ...
        except (NotFoundError, OrchestratorError):
            continue
