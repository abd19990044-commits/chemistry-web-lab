...
from .errors import (AuthenticationError, ConcurrencyError, IllegalTransitionError,
                     KaggleUnavailableError, LeaseLostError, NotFoundError,
                     OrchestratorError, PermanentError, QuotaExhaustedError, RateLimitError)
...
class Observation:
    ...
    successor_status: KernelStatus | None = None
...
def observe(client: KaggleClient, job: JobManifest) -> Observation:
    ...
    if status.is_stopped:
        try:
            next_slug = job.slug_for_epoch(job.epoch + 1)
            obs.successor_status = client.kernel_exists(next_slug)
        except OrchestratorError:
            obs.successor_status = None
...
def decide(job: JobManifest, obs: Observation, *, config=CONFIG) -> Decision:
    ...
    if job.is_terminal:
        if obs.kaggle_state == "error":
            raw_msg = (obs.kernel_status.raw if obs.kernel_status else "") or "kernel failed on Kaggle"
            return Decision(Trigger.ORCA_FATAL, "kernel errored on Kaggle",
                            {"note": raw_msg, "error": raw_msg}, action="adopt_ledger")
        return _noop("job is in a terminal state", state=job.state.value)

    if obs.successor_status is not None:
        next_slug = job.slug_for_epoch(job.epoch + 1)
        return Decision(Trigger.SUCCESSOR_PUSHED,
                        "the successor kernel already exists on Kaggle",
                        {"next_slug": next_slug}, action="adopt_ledger")

    if obs.error is not None:
        note = "Kaggle output files rate-limited; will retry on next poll." if isinstance(obs.error, RateLimitError) else None
        return _noop("observation failed; the job's state is unchanged because a "
                     "failure to *look* is not a failure of the job",
                     error=obs.error.code, note=note)
    ...
    if kaggle_state == "complete" and ledger_job is None and job.epoch == 0 and job.verified_checkpoint_id is None:
        note = (ledger.note if (ledger and ledger.note) else "Calculation complete")
        return Decision(Trigger.ORCA_COMPLETE,
                        "a pre-orchestrator window completed",
                        {"note": note}, action="adopt_ledger")

    if kaggle_state == "error" and ledger_job is None and job.epoch == 0 and job.verified_checkpoint_id is None:
        raw_msg = (obs.kernel_status.raw if obs.kernel_status else "") or "kernel failed on Kaggle"
        return Decision(Trigger.ORCA_FATAL,
                        "kernel errored on Kaggle",
                        {"note": raw_msg, "error": raw_msg}, action="adopt_ledger")
...
class Reconciler:
    ...
    def reconcile(self, job_id: str, creds: KaggleCredentials, *,
        ...
        with log_context(correlation_id=correlation_id, job_id=job_id):
            with self.store.lease(f"job:{job_id}", holder) as lease:
                ...
                if obs.kernel_missing and job.current_slug != job.job_id:
                    try:
                        base_status = client.kernel_exists(job.job_id)
                        ...
                        pass
                if obs.error is not None and isinstance(obs.error, (AuthenticationError, KaggleUnavailableError)):
                    raise obs.error
    ...
    def _act(self, job: JobManifest, decision: Decision, obs: Observation,
        ...
        if decision.action == "adopt_ledger":
