    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        for key, value in record.__dict__.items():
            if key in self._RESERVED or key.startswith("_"):
                continue
            clean_key = key[6:] if key.startswith("extra_") else key
...
def _sanitize_extra(d: dict[str, Any]) -> dict[str, Any]:
    return {
        (f"extra_{k}" if k in _RESERVED_LOG_RECORD_ATTRS else k): v
        for k, v in d.items()
    }
...
def log_failure(
    ...
    **fields: Any,
    ...
    logger.error("%s failed: %s", what, why, extra=_sanitize_extra(extra), exc_info=exc)
...
def log_event(logger: logging.Logger, event: str, message: str = "", **fields: Any) -> None:
    ...
    logger.info(message or event, extra=_sanitize_extra(extra))
