...
CONVERGENCE_MARKERS = (
    ...
    # ORCA 6 variants (lowercase-normalised via _normalize before matching)
...
def validate_gbw(path: str) -> ValidationResult:
    ...
    Restart is therefore driven entirely from artefacts that CAN be verified - .xyz, *_trj.xyz, .allxyz, .hess, .mdrestart, .res.* - and `! NoAutoStart`
...
def clamp_maxcore(text: str, nprocs: int, total_ram_mb: int,
                  fraction: float = 1.0) -> tuple[str, dict | None]:
    """`%maxcore` is PER MPI PROCESS. Clamping is applied only when the requested total
    memory exceeds the 30 GB (30720 MB) physical limit."""
    hard_limit_mb = min(total_ram_mb, 30720) if total_ram_mb >= 30000 else total_ram_mb
