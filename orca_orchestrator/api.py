...
def _error(exc, status):
    payload = {"ok": False, "error": getattr(exc, "message", str(exc)), **exc.to_dict()}
    if "error" not in payload: payload["error"] = getattr(exc, "message", str(exc))
...
def workflows():
    payload = _json()
    creds = _credentials_from(payload if request.method == "POST" else request.args)
    service = get_service()
    if request.method == "POST" and payload.get("title") and payload.get("steps"):
        title = str(payload["title"]).strip()
        raw_datasets = payload.get("dataset_sources") or ""
        try:
...
@bp.route("/workflows/<workflow_id>", methods=["GET", "POST"])
def workflow_detail(workflow_id):
    payload = _json()
    creds = _credentials_from(payload if request.method == "POST" else request.args)
    service = get_service()
    wf = service.get_workflow(creds, workflow_id)
    if wf is None:
...
def results():
    ...
    if file_size <= 100 * 1024 * 1024:
        ...
        return send_file(data_bytes, as_attachment=True, download_name=f"{slug or job_id}_results.zip", mimetype="application/zip")
    else:
        response = send_file(zip_path, as_attachment=True, download_name=f"{slug or job_id}_results.zip", mimetype="application/zip")
        if cleanup_dir:
...
def archive():
    ...
    service = get_service()
    manifest = service.archive_job_results(creds, job_id, slug=slug)
    if not manifest:
        return jsonify({"ok": False, "error": "Could not archive results", "job_id": job_id}), 502
    return jsonify({"ok": True, "job_id": job_id, "manifest": manifest.to_dict()})
...
def sweep():
    ...
    creds = _credentials_from(payload)
    service = get_service()
    auth_creds = service.ensure_authenticated(creds)
    with log_context(correlation_id=request.environ.get("orca.correlation_id", "-")):
        return jsonify({"ok": True, "owner": auth_creds.username, "sweep": service.sweep_now(owner=auth_creds.username)})
...
def register(app, *, start_watchdog=True):
    """Register canonical orchestrator routes at /api/orca/*."""
    app.register_blueprint(bp)
    app.errorhandler(OrchestratorError)(_handle_orchestrator_error)
