...
class TestMetadata:
    ...
    def test_basis_version_and_solvation_are_captured(self) -> None:
        ...
        assert job.metadata.orca_version == "6.1.0"
        assert job.metadata.basis_set == "def2-TZVP"
        assert job.metadata.solvation == "SMD"
...
class TestConceptualDFT:
    ...
    def test_largeprint_molecular_orbitals_block(self) -> None:
        """Parse MOLECULAR ORBITALS matrix block emitted under LargePrint."""
        ...
        lines = iter(
            [
                ...
                "------------------",
                "                      0         1         2",
                "                  -0.30000  -0.20000  -0.05000",
                "                     2.0000    2.0000    0.0000",
                "                  --------- --------- ---------",
                "  0 C   1s          0.99234   0.01234  -0.00345",
                "  1 H   1s          0.00123   0.54321   0.23456",
                "****ORCA TERMINATED NORMALLY****",
            ]
        )
        job = OrcaParser(lines).parse()[-1]
        assert job.homo_ev == pytest.approx(-0.20000 * 27.211386245988)
        assert job.lumo_ev == pytest.approx(-0.05000 * 27.211386245988)
        assert job.homo_lumo_gap_ev == pytest.approx(0.15000 * 27.211386245988)

    def test_orbital_energies_table_without_ev_column(self) -> None:
        """Parse ORBITAL ENERGIES table when only Hartree column is printed."""
        ...
        job = OrcaParser(lines).parse()[-1]
        assert job.homo_ev == pytest.approx(-0.250000 * 27.211386245988)
        assert job.lumo_ev == pytest.approx(-0.050000 * 27.211386245988)

    def test_hirshfeld_charges_parsing(self) -> None:
        """Parse HIRSHFELD ANALYSIS charge section."""
        ...
        job = OrcaParser(lines).parse()[-1]
        assert len(job.hirshfeld_charges) == 3
        assert job.hirshfeld_charges[0] == pytest.approx(-0.123456)
        assert job.hirshfeld_charges[1] == pytest.approx(0.123456)
        assert job.hirshfeld_charges[2] == pytest.approx(-0.456789)
        assert job.atomic_charges["hirshfeld"] == job.hirshfeld_charges

    def test_mulliken_charges_table_parsing(self) -> None:
        """Parse MULLIKEN ATOMIC CHARGES table."""
        ...
        lines = iter(
            [
                ...
                "------------------------",
                "   0 C   -0.250000    0.000000",
                "   1 H    0.250000    0.000000",
                "Sum of atomic charges: 0.000000",
                "****ORCA TERMINATED NORMALLY****",
            ]
        )
        job = OrcaParser(lines).parse()[-1]
        assert len(job.mulliken_charges) == 2
        assert job.mulliken_charges[0] == pytest.approx(-0.25)
        assert job.mulliken_charges[1] == pytest.approx(0.25)

    def test_mayer_population_analysis_parsing(self) -> None:
        """Parse MAYER POPULATION ANALYSIS charges (QA) and valencies (VA)."""
        ...
        lines = iter(
            [
                ...
                "  0 C      5.8564     6.0000     0.1436     3.9123     3.9123     0.0000",
                "  1 H      0.8712     1.0000     0.1288     0.9741     0.9741     0.0000",
                "  2 O      8.4522     8.0000    -0.4522     2.0115     2.0115     0.0000",
                "****ORCA TERMINATED NORMALLY****",
            ]
        )
        job = OrcaParser(lines).parse()[-1]
        assert len(job.mayer_charges) == 3
        assert job.mayer_charges[0] == pytest.approx(0.1436)
        assert job.mayer_charges[1] == pytest.approx(0.1288)
        assert job.mayer_charges[2] == pytest.approx(-0.4522)
        assert len(job.mayer_valences) == 3
        assert job.mayer_valences[0] == pytest.approx(3.9123)
        assert job.atomic_charges["mayer"] == job.mayer_charges
        assert job.atomic_charges["mayer_valence"] == job.mayer_valences
