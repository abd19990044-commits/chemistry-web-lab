...
class TestCommandLine:
    ...
    def test_parse_molden_file(self, tmp_path: Path) -> None:
        """Parse atoms and Hirshfeld charges from a .molden file."""
        from orca_engine.io import parse_path_safe

        molden_file = tmp_path / "water.molden"
        molden_file.write_text(
            """[Molden Format]
[Title]
Water Molecule Molden Export
[Atoms] Angs
...
""",
        ...
        molecules = parse_path_safe(molden_file)
        assert len(molecules) == 1
        mol = molecules[0]
        job = mol.jobs[0]
        assert len(job.elements) == 3
        assert job.elements == ["O", "H", "H"]
        assert len(job.hirshfeld_charges) == 3
        assert job.hirshfeld_charges[0] == pytest.approx(-0.42)
        assert job.hirshfeld_charges[1] == pytest.approx(0.21)
