...
def job_to_dict(index: int, job: JobData) -> dict[str, Any]:
    ...
    return {
        ...
        "electronic_energy_hartree": job.e_elec_eh,
        "total_energy_eh": job.e_elec_eh,
        "scf_energy_hartree": job.e_elec_eh,
        "zpe_eh": job.zpe_eh,
        "zero_point_energy_hartree": job.zpe_eh,
        "zpe_hartree": job.zpe_eh,
        "electronic_zpe_eh": job.electronic_zpe_eh(),
        "gibbs_free_energy_eh": job.gibbs_free_energy_eh,
        "gibbs_free_energy_hartree": job.gibbs_free_energy_eh,
        "final_gibbs_hartree": job.gibbs_free_energy_eh,
        "total_enthalpy_eh": job.total_enthalpy_eh,
        "enthalpy_hartree": job.total_enthalpy_eh,
        "final_enthalpy_hartree": job.total_enthalpy_eh,
        ...
        "stationary_point_status": job.stationary_point_status,
        "thermochemistry_reliability": job.thermochemistry_reliability,
    }
...
def consistency_to_dict(report: ConsistencyReport) -> dict[str, Any]:
    ...
    return {
        ...
        "levels_of_theory": [
            {"orca_version": version, "method": method, "basis_set": basis, "dispersion": disp, "solvation": solvation, "solvent": solv}
            for method, basis, disp, solvation, solv, version in report.mixed_levels_of_theory
        ],
    ...
    report: dict[str, Any] = {
        "molecules": {
            name: {
                ...
                "levels_of_theory": [
                    {"orca_version": version, "method": method, "basis_set": basis, "dispersion": disp, "solvation": solvation, "solvent": solv}
                    for method, basis, disp, solvation, solv, version in molecule.levels_of_theory()
                ],
...
def reaction_result_to_dict(result: ReactionResult) -> dict[str, Any]:
    ...
    return {
        ...
        "delta_entropy_j_mol_k": (result.delta_entropy_cal_mol_k * PhysConst.CAL_TO_JOULE) if result.delta_entropy_cal_mol_k is not None else None,
        "delta_s_cal_mol_k": result.delta_entropy_cal_mol_k,
        "delta_s_j_mol_k": (result.delta_entropy_cal_mol_k * PhysConst.CAL_TO_JOULE) if result.delta_entropy_cal_mol_k is not None else None,
        "equilibrium_constant_keq": result.equilibrium_constant_keq,
        "keq_standard": result.keq_standard,
        "keq_pressure_corrected": result.keq_pressure_corrected,
        "temperature_k": result.temperature_k,
        "temperature_requested_k": result.temperature_requested_k,
        "delta_g_requested_kcal_mol": result.delta_g_requested_kcal_mol,
        "keq_requested": result.keq_requested,
        "temperature_treatment_mode": result.temperature_treatment_mode,
        "pressure_atm": result.pressure_atm,
        "pressure_correction_applied": result.pressure_correction_applied,
        "delta_g_pressure_corrected_kcal_mol": result.delta_g_pressure_corrected_kcal_mol,
        "is_exergonic": result.is_exergonic,
        "missing_references": result.missing_references,
        "composite_provenance": result.composite_provenance,
