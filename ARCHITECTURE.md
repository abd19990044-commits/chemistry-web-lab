...
> calls `/api/orca/coords`, `/api/orca/coords/file` and `/api/orca/generate` - > all three of which are defined in `app.py` itself - and never calls
...
The lifecycle suite replaces only `KaggleClient`. Everything below it is real - including `build_window_directory`, so the generated Kaggle script is compiled
...
Recorded in full because it is the most instructive failure in this document - the bug was not in the checkpoint logic at all, and it produced a symptom that
...
succeeds, the database still works correctly in rollback-journal mode - concurrency drops, nothing is lost. Degrading beats refusing to start. The
...
it; the first removed it; the second's `os.remove()` raised `FileNotFoundError` - a subclass of `OSError`, caught by the same handler that means *"this
...
## 12. Browser-Side Archive & File Classification Pipeline

### 12.1 Purpose & Scope
The browser-side file pipeline offloads archive decompression (ZIP, TAR, GZ, TGZ), SHA-256 deduplication, and 6-tier file classification to a dedicated Web Worker (`static/js/workers/archive-worker.js`).

**Strict Separation of Concerns**:
* The client-side pipeline applies **specifically and exclusively to user-uploaded external analysis datasets** (e.g. historical ORCA outputs and chemical structures loaded for visualization, property analysis, or thermochemistry).
* The **server remains the single source of truth** for all persistent jobs, Kaggle session orchestration, checkpointing, retries, and ledger records.

### 12.2 Security, Limits & Decompression Bomb Defense
To protect browser memory and user experience without causing UI freezes:
1. **Configurable Adaptive Limits**:
   * Maximum Archive Size: 500 MB.
   * Maximum Extracted Dataset Size: 1 GB.
   * Maximum Individual File Size: 250 MB.
   * Maximum File Count: 2,000 files.
   * Maximum Compression Ratio: 100× (automatic rejection of suspicious decompression bombs).
2. **Zip Slip & Path Traversal Neutralization**:
   * All file paths extracted from archives are strictly sanitized via `sanitizePath()`: absolute paths, leading slashes, Windows drive letters (`C:`), and directory traversal segments (`../`, `..\\`) are stripped.
3. **Web Crypto SHA-256 Deduplication**:
   * Uses native `crypto.subtle.digest("SHA-256")` in the worker thread to identify identical files, preserving the first instance for analysis and tagging subsequent copies as `DUPLICATE` with explicit source provenance.

### 12.3 6-Tier File Classification Schema
Every discovered file is deterministically categorized into one of six tiers:
* `ANALYZE`: Supported quantum and chemical formats ready for direct property extraction (`.out`, `.log`, `.property.txt`, `.xyz`, `.sdf`, `.mol`).
* `KEEP`: Ancillary scientific input files preserved in the dataset (`.inp`, `.hess`, `.allxyz`, `.pdb`, `.cube`).
* `IGNORE`: OS/system metadata and temporary noise excluded from the queue (`__MACOSX/`, `.DS_Store`, `Thumbs.db`, `README*`, `LICENSE*`, `*.pyc`, `*.tmp`). Excluded from calculation queues, never deleted.
* `DUPLICATE`: Redundant duplicate files identified by SHA-256 hash match.
* `UNKNOWN`: Unrecognized text/ASCII files. **`UNKNOWN` is never deleted automatically**; it is surfaced with an inspection badge (`? UNKNOWN`) for user review and manual execution.
* `UNSUPPORTED`: Non-computational binaries and media formats (`.exe`, `.dll`, `.png`, `.mp4`).
