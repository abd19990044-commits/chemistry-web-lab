...
def _run_cli_cmd(args, **kwargs):
    cmd = list(args)
    # Ensure UTF-8 execution environment for all Kaggle CLI calls
    env = kwargs.get("env")
    if env is None:
        env = os.environ.copy()
        kwargs["env"] = env
...
_ORIGINAL_RUN_CLI = _run_kaggle_cli
...
def cli_health() -> dict:
    ...
    path = sys.executable
    try:
        proc = _run_cli_cmd(["kaggle", "--version"], capture_output=True,
...
def _clamp_maxcore(text, nprocs):
    # %maxcore is PER MPI PROCESS. Clamping is only applied if total requested
    # memory exceeds 30 GB (30720 MB) or the physical machine capacity.
    global MAXCORE_NOTE
    MAXCORE_NOTE = ""            # a later re-clamp (serial retry) supersedes an earlier one
    total_mb = _total_ram_mb()
    hard_limit_mb = min(total_mb, 30720) if total_mb >= 30000 else total_mb
    cap = max(700, hard_limit_mb // max(1, nprocs))
    changed = []

    def repl(m):
        want = int(m.group(2))
        if (want * max(1, nprocs)) <= hard_limit_mb:
            return m.group(0)
    ...
    if changed:
        want, got = changed[0]
        MAXCORE_NOTE = (
            "Memory: the input requested %%maxcore %d MB per process x %d process(es) "
            "= %d MB, exceeding the maximum machine limit of %d MB. It was capped at "
            "%d MB per process (%d MB total)."
            % (want, nprocs, want * nprocs, hard_limit_mb, got, got * nprocs)
        )
...
def _temp_kaggle_env(username: str, key: str):
    ...
    try:
        ...
        if auth["api_token"]:
            clean_key = auth["api_token"][5:] if auth["api_token"].startswith("KGAT_") else auth["api_token"]
            env["KAGGLE_API_TOKEN"] = auth["api_token"]
            env["KAGGLE_KEY"] = clean_key
            with open(os.path.join(cfg_dir, "access_token"), "w") as fh:
            ...
            try:
                ...
                pass
            with open(os.path.join(cfg_dir, "kaggle.json"), "w") as fh:
                json.dump({"username": auth["username"], "key": clean_key}, fh)
            try:
                os.chmod(os.path.join(cfg_dir, "kaggle.json"), 0o600)
            except OSError:
                pass
...
_KAGGLE_DATASET_URL_RE = re.compile(
    r"^(?:https?://)?(?:[a-zA-Z0-9_-]+\.)?kaggle\.com/(?:datasets/)?([A-Za-z0-9._-]+)/([A-Za-z0-9._-]+)(?:[/?#].*)?$",
    re.IGNORECASE,
)
_KAGGLE_DATASET_ID_RE = re.compile(r"^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$")


def clean_dataset_source(raw: str) -> str:
    """Normalizes a Kaggle dataset identifier or URL into 'owner/dataset-slug'.

    Accepts:
    - Full Kaggle dataset URLs (e.g. 'https://www.kaggle.com/datasets/owner/dataset-slug')
    - Short Kaggle URLs (e.g. 'kaggle.com/datasets/owner/dataset-slug')
    - Direct canonical identifiers (e.g. 'owner/dataset-slug')

    Strips protocol, domain, '/datasets/' prefix, trailing slashes, query parameters,
    fragments, and subpaths.

    Raises:
        ValueError: If input is an unrelated URL, malformed URL, or invalid identifier format.
    """
    raw = (raw or "").strip().strip('"').strip("'")
    if not raw:
        return ""

    if _KAGGLE_DATASET_ID_RE.match(raw):
        return raw

    m = _KAGGLE_DATASET_URL_RE.match(raw)
    if m:
        owner = m.group(1)
        dataset_slug = m.group(2)
        if owner.lower() in ("datasets", "code", "competitions", "discussions", "models", "learn", "settings"):
            raise ValueError(f"Invalid Kaggle dataset URL '{raw}': '{owner}' is a reserved Kaggle route.")
        return f"{owner}/{dataset_slug}"

    if raw.startswith(("http://", "https://", "www.")) or "kaggle.com" in raw:
        raise ValueError(
            f"Invalid Kaggle dataset URL '{raw}'. Expected format: https://www.kaggle.com/datasets/owner/dataset-slug"
        )

    parts = raw.split("/")
    if len(parts) != 2 or not parts[0].strip() or not parts[1].strip():
        raise ValueError(
            f"Invalid Kaggle dataset identifier '{raw}'. Expected 'owner/dataset-slug' format."
        )

    raise ValueError(
        f"Invalid Kaggle dataset identifier '{raw}'. Dataset identifiers must contain only letters, numbers, dots, hyphens, and underscores."
    )
...
def push_job(job_dir: str, kaggle_username: str, kaggle_key: str) -> dict:
    ...
    with _temp_kaggle_env(kaggle_username, kaggle_key) as env:
        temp_dir = env.get("KAGGLE_CONFIG_DIR", tempfile.gettempdir())
        push_script = os.path.join(temp_dir, "push_kernel.py")
        with open(push_script, "w", encoding="utf-8") as fh:
            fh.write(
                ...
                "    api = KaggleApi()\n"
                "    api.authenticate()\n"
                f"    job_folder = r'{job_dir}'\n"
                "    resp = api.kernels_push(job_folder)\n"
                "    err_msgs = []\n"
                "    if getattr(resp, 'error', None):\n"
                "        err_msgs.append(str(resp.error))\n"
                "    if getattr(resp, 'invalid_dataset_sources', None):\n"
                "        err_msgs.append('Invalid dataset source(s): ' + ', '.join(resp.invalid_dataset_sources))\n"
                "    if getattr(resp, 'invalid_kernel_sources', None):\n"
                "        err_msgs.append('Invalid kernel source(s): ' + ', '.join(resp.invalid_kernel_sources))\n"
                "    if getattr(resp, 'invalid_competition_sources', None):\n"
                "        err_msgs.append('Invalid competition source(s): ' + ', '.join(resp.invalid_competition_sources))\n"
                "    if err_msgs:\n"
                "        print('ERR|' + ' ; '.join(err_msgs))\n"
                "        sys.exit(1)\n"
                "    url = getattr(resp, 'url', None)\n"
                "    ref = getattr(resp, 'ref', None)\n"
                "    if not url and not ref:\n"
                "        print('ERR|Kaggle API returned empty response without kernel URL or ref.')\n"
                "        sys.exit(1)\n"
                "    final_url = url or f'https://www.kaggle.com/code/{ref}'\n"
                "    print('URL|' + str(final_url))\n"
                "    if ref:\n"
                "        print('REF|' + str(ref))\n"
        ...
        try:
            # Increase push timeout to 240 seconds to prevent premature disconnections
            ...
            result = None
            combined = f"ERR|Kaggle push timed out locally after 240 seconds: {exc}"
        except Exception as exc:
            result = None
            combined = f"ERR|Unexpected local error during push: {exc}"

        try:
            import time
            log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "last_push_debug.log")
        ...
        if result is not None:
            for line in combined.splitlines():
                ...
                elif line.startswith("AUTH_USER|"):
                    auth_user = line.split("AUTH_USER|", 1)[1].strip()

        owner = (auth_user or auth["username"]).lower()
        slug = requested_slug
        kernel_verified = False

        # Smart fallback: if timeout or transient error occurs, probe Kaggle status directly before failing
        if result is None or result.returncode != 0 or "ERR|" in combined or not url:
            import time
            time.sleep(3)
            try:
                probe = _run_kaggle_cli(
                    ["kaggle", "kernels", "status", f"{owner}/{slug}"],
            ...
            if not kernel_verified:
                ...
                if "ERR|" in message:
                    message = message.split("ERR|")[-1].split("\n")[0].strip()
                if "401" in message or "Unauthorized" in message or "Authentication required" in message or "403" in message or "Forbidden" in message:
                    message += (
                        "\n\nThis usually means: the API key/token was pasted with a typo, an old key was regenerated, or the 'Kaggle username' doesn't match the key."
                    )
                raise RuntimeError(f"Kaggle push failed: {message}")
            else:
                print(f"Push reported an error/timeout, but kernel {owner}/{slug} was verified to exist. Proceeding smoothly.")
        
        import re
        if url and not kernel_verified:
            m = re.search(r"kaggle\.com/(?:code/)?([^/]+)/([^/\s]+)", url)
            if m:
                owner, slug = m.groups()
            elif ref and "/" in ref:
    ...
    return {
        "job_id": slug,
        "owner": owner,
        "url": url if url and url.startswith("http") else f"https://www.kaggle.com/code/{owner}/{slug}",
    }
    ...
    with _temp_kaggle_env(kaggle_username, kaggle_key) as env:
        ...
        if result.returncode != 0:
            ...
            if "401" in combined or "unauthorized" in low or "authentication required" in low or "forbidden" in low:
                raise RuntimeError("Kaggle rejected these credentials. Check the username and API key/token on kaggle.com -> Settings -> API.")
...
def list_jobs(kaggle_username: str, kaggle_key: str) -> list[dict]:
    ...
    elsewhere from a differently-sanitized copy of the same username - e.g. a hyphen in a real Kaggle username like 'chem-lab-99' silently
    ...
    if True:
        ...
        jobs.sort(key=lambda j: str(j.get("last_run") or ""), reverse=True)
        return jobs
...
def check_job_status(kaggle_username: str, kaggle_key: str, job_id: str) -> dict:
    ...
    with _temp_kaggle_env(kaggle_username, kaggle_key) as env:
        ...
        if status_result.returncode != 0 and status == "unknown":
            ...
            if "401" in lowered or "unauthorized" in lowered or "authentication" in lowered or "forbidden" in lowered:
                note += ("\n\nCheck your Kaggle username and API key/token - go to "
                         "kaggle.com -> Settings -> API to view or regenerate it.")
                return {"status": "error", "next_job_id": None, "next_kaggle_url": None, "note": note}
            elif "404" in lowered or "not found" in lowered:
                # Kaggle can take 5-15s to register newly pushed kernels in its backend.
                return {
                    "status": "queued",
                    "next_job_id": None,
                    "next_kaggle_url": None,
                    "note": "Notebook is initializing on Kaggle cloud..."
                }
            return {"status": "unknown", "next_job_id": None, "next_kaggle_url": None, "note": note}
        ...
        if status in ("complete", "error", "cancelled"):
            ...
            try:
                ...
                if handoff.returncode != 0:
                    ...
                    handoff_text = (handoff.stderr or handoff.stdout or "").strip()
                    return {"status": "restarting" if status == "complete" else (status if status != "unknown" else "unknown"),
                            "next_job_id": None,
                            "next_kaggle_url": None,
                            "note": ("Kaggle rate-limited output files listing; will retry on next poll. %s" % handoff_text).strip()}
...
def fetch_job_results(kaggle_username: str, kaggle_key: str, job_id: str):
    ...
    if result.returncode != 0:
        ...
        # Check if the kernel is currently running or queued on Kaggle
        try:
            st = check_job_status(kaggle_username, kaggle_key, job_id)
            if st.get("status") in ("running", "queued", "submitting", "restarting"):
                raise RuntimeError(
                    f"Job '{job_id}' is currently {st['status']} on Kaggle. "
                    "Results will be available to download and analyze once the calculation completes."
                )
            if st.get("status") == "error":
                raise RuntimeError(
                    f"Kaggle reported that job '{job_id}' encountered an error. {st.get('note', '')}".strip()
                )
        except (KaggleCliUnavailable, KaggleUnreachable, RuntimeError):
            raise
        except Exception:
            pass
        ...
        if "404" in message.lower() or "not found" in message.lower():
            raise RuntimeError(
                f"No output bundle found on Kaggle for job '{job_id}'. "
                "If it recently started, please wait for it to finish."
            )
        raise RuntimeError(message)
    ...
    with zipfile.ZipFile(fallback_zip, "w", zipfile.ZIP_DEFLATED) as zf:
        for f in glob.glob(os.path.join(out_dir, "*")):
            ...
            if not os.path.isfile(f) or f in (fallback_zip, zip_path):
                continue
