...
class FakeClient:
    ...
    def __init__(self, creds=None, fake=None):
        self.creds = creds or CREDS
        self.fake = fake

    def kernel_exists(self, slug):
        status = self.fake.kernels.get(slug) if self.fake else None
        return None if status is None else KernelStatus(slug, status)

    def status(self, slug):
        return KernelStatus(slug, self.fake.kernels.get(slug, "unknown") if self.fake else "unknown")
    ...
    def push_kernel(self, job_dir, *, expected_slug, skip_if_active=True):
        ...
        if skip_if_active and self.fake and self.fake.kernels.get(expected_slug) in ("running", "queued"):
            self.fake.push_log.append((expected_slug, "skipped"))
            return PushResult(slug=expected_slug, owner="tester",
                              url="https://kaggle/%s" % expected_slug,
                              requested_slug=expected_slug)

        if self.fake and self.fake.fail_next_push:
            err = self.fake.fail_next_push
            self.fake.fail_next_push = None
            raise err

        if self.fake:
            self.fake.push_log.append((expected_slug, "pushed"))
            self.fake.kernels[expected_slug] = "queued"
        return PushResult(slug=expected_slug, owner="tester",
                          url="https://kaggle/%s" % expected_slug,
                          requested_slug=expected_slug)
    ...
    def list_kernels(self, prefix=None, page_size=100):
        return [{"slug": s, "owner": "tester", "ref": "tester/" + s,
                ...
                for s in (self.fake.kernels if self.fake else [])]

    def delete_kernel(self, slug):
        if self.fake:
            self.fake.kernels.pop(slug, None)
        return True
...
def run_all_checks():
    ...
    class BoundFakeClient(FakeClient):
        def __init__(self, creds=None):
            super().__init__(creds=creds, fake=fake)

    def fake_read_window(client, slug):
        return fake.ledgers.get(slug) or LedgerRecord(slug=slug)

    orig_client = reconciler_mod.KaggleClient
    orig_read = reconciler_mod.read_window
    reconciler_mod.KaggleClient = BoundFakeClient
    ...
    try:
        ...
        job = rec.reconcile(JOB_ID, CREDS)
        check("a window that verified a checkpoint but died before pushing gets its successor pushed",
              job.epoch == 1, "epoch=%d state=%s" % (job.epoch, job.state.value))
        ...
        pushes_before = len(fake.push_log)
        job = rec._push_successor(store.require_job(JOB_ID), BoundFakeClient(), fence=1,
        ...
        fake.ledgers.pop(JOB_ID + "-r1", None)
        ...
        store.put_job(b)
        b = rec._push_successor(store.require_job(BUDGET_JOB), BoundFakeClient(), fence=1,
                                correlation_id="t", actor="test")
        ...
        store.put_job(c)
        c = rec._push_successor(store.require_job(CYCLE_JOB), BoundFakeClient(), fence=1,
                                correlation_id="t", actor="test")
        ...
        o.epoch = 1
        store.put_job(o)
        o = rec._rollback(store.require_job(ORPHAN), BoundFakeClient(), fence=1,
                          correlation_id="t", actor="test")
        check("with nothing verified to fall back to, the job fails rather than silently restarting from zero",
              o.state is JobState.FAILED, o.state.value)
        ...
        reconciler_mod.KaggleClient = orig_client
        reconciler_mod.read_window = orig_read
