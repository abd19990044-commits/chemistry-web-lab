...
Run: `python tests/test_deployment.py` or `pytest tests/test_deployment.py`
"""
import os
import re
import subprocess
...
def run_deployment_checks():
    ...
    try:
        ...
        if health["ok"]:
            check("the installed CLI is supported (1.x or 2.x series)",
                  any(v in health.get("version", "") for v in ("1.", "2.")), health.get("version"))
        else:
    ...
    print("\n" + "=" * 70)
    print("DEPLOYMENT: %d passed, %d failed, %d skipped" % (passed, failed, skipped))
    print("=" * 70)
    return passed, failed, skipped


def test_deployment():
    passed, failed, skipped = run_deployment_checks()
    assert failed == 0, f"{failed} deployment checks failed"
