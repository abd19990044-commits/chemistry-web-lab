...
aborted the rest of the script. Every handler registered after that point - including the Kaggle sign-in form's - was never attached, so signing in silently
...
def run_checks():
    ...
    known_ids = html_ids | dynamic_ids
    wanted = set(re.findall(r'getElementById\(\s*"([^"]+)"\s*\)', js))
    missing = sorted(wanted - known_ids)
    check("no getElementById targets an id that is not in index.html or dynamic templates",
    ...
    check("two ORCA source options are present (dataset, link)",
          "orca-source-dataset" in html and "orca-source-link" in html)
    check("ORCA .tar.xz archive upload UI is completely removed",
          "orca-source-upload" not in html and "kaggle-orca-upload-file" not in html and "orca-upload-status-card" not in html)
    check("completed jobs display canonical Full ZIP button",
          "📦 Full ZIP" in js)
    check("Fast Results button is completely removed",
          "Fast Results" not in js and "downloadEssentialBtn" not in js)
...
class EventTargetMock {
  ...
  dispatchEvent(event) {
    const type = (event && typeof event === 'object' && event.type) ? event.type : String(event);
...
function el(id) {
  ...
  return new Proxy(target, {
    get(_t, prop) {
      if (prop in et || typeof et[prop] === 'function') {
        return typeof et[prop] === 'function' ? et[prop].bind(et) : et[prop];
      }
...
global.window = Object.assign(windowEvents, {
  location: { href: '', search: '', pathname: '/', origin: 'http://localhost' },
  matchMedia: () => ({ matches: false, addEventListener() {}, removeEventListener() {}, dispatchEvent() { return true; } }),
  scrollTo() {},
  Event: global.Event,
...
"""
        ...
        try:
            ...
            class ServerThread(threading.Thread):
                def __init__(self, srv_app, port=5125):
                    super().__init__(daemon=True)
                    self.server = make_server("127.0.0.1", port, srv_app)
                    self.ctx = srv_app.app_context()
                    self.ctx.push()
                def run(self):
            ...
            srv = ServerThread(webapp.app, port=5125)
            srv.start()
            time.sleep(0.8)
            ...
            try:
                driver.get("http://127.0.0.1:5125/")
                time.sleep(1.2)
                logs = driver.get_log("browser")
                severe = [
                    l for l in logs
                    if l.get("level") == "SEVERE"
                    and "fonts.googleapis.com" not in l.get("message", "")
                    and "fonts.gstatic.com" not in l.get("message", "")
                ]
                fn_type = driver.execute_script("return typeof window.showChemistryView;")
                check("REGRESSION: app.js evaluates in browser with 0 severe errors",
                      len(severe) == 0, "\n".join(l["message"] for l in severe))
                check("...and registers global router window.showChemistryView",
                      fn_type == "function", "got: %s" % fn_type)
            finally:
                driver.quit()
                srv.shutdown()
        except ImportError:
            print("  SKIP  neither node nor selenium is installed in this Python env; static checks passed")
        except Exception as exc:
            check("Selenium headless evaluation ran", False, str(exc))
...
def test_suite():
    _passed, _failed = run_checks()
    assert _failed == 0, f"{_failed} checks failed"
